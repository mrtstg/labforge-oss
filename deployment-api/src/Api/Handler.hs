...
handleTask _ (GroupDeployment tID groupName authorID) = do
  ...
  case groupMembersResp of
    ...
    (Right users) -> do
      ...
      mapM_ (\(Entity (DeploymentInstanceDataKey t) (DeploymentInstanceData { .. })) -> withTokenVariable $ \token -> defaultRetryClient jobserviceEnv $ J.insertJobserviceMessage' (JobserviceDeployInstance {}) (Just JobserviceMessageMeta {targetUserId=Just deploymentInstanceDataOwnerId, actionGroup=Just groupKey, authorId=authorID, deploymentId=t, templateId=(fromIntegral . fromSqlKey) deploymentInstanceDataParent}) (BearerWrapper token)) existingDeployments
...
handleTask _ (GroupRollback tID groupName snapName mask) = do
  ...
  case groupMembersResp of
    ...
    (Right users) -> do
      ...
      jobserviceEnv <- asks $ getEnvFor JobserviceAPI
      mapM_ (\(Entity (DeploymentInstanceDataKey t) (DeploymentInstanceData { .. })) -> withTokenVariable $ \token -> defaultRetryClient jobserviceEnv $ J.insertJobserviceMessage' (JobserviceRollback snapName mask) (Just JobserviceMessageMeta {targetUserId=Just deploymentInstanceDataOwnerId, actionGroup=Nothing, authorId=Nothing, deploymentId=t, templateId=(fromIntegral . fromSqlKey) deploymentInstanceDataParent}) (BearerWrapper token)) existingDeployments
handleTask _ (GroupMakeSnapshot tID groupName snapName mask) = do
  ...
  case groupMembersResp of
    ...
    (Right users) -> do
      ...
      jobserviceEnv <- asks $ getEnvFor JobserviceAPI
      mapM_ (\(Entity (DeploymentInstanceDataKey t) (DeploymentInstanceData { .. })) -> withTokenVariable $ \token -> defaultRetryClient jobserviceEnv $ J.insertJobserviceMessage' (JobserviceSnapshot snapName False mask "") (Just JobserviceMessageMeta {targetUserId=Just deploymentInstanceDataOwnerId, actionGroup=Nothing, authorId=Nothing, deploymentId=t, templateId=(fromIntegral . fromSqlKey) deploymentInstanceDataParent}) (BearerWrapper token)) existingDeployments
...
handleTask _ (GroupPower tID groupName powerOn mask) = do
  ...
  case groupMembersResp of
    ...
    (Right users) -> do
      ...
      jobserviceEnv <- asks $ getEnvFor JobserviceAPI
      mapM_ (\(Entity (DeploymentInstanceDataKey t) (DeploymentInstanceData { .. })) -> withTokenVariable $ \token -> defaultRetryClient jobserviceEnv $ J.insertJobserviceMessage' (JobservicePower powerOn mask) (Just JobserviceMessageMeta {targetUserId=Just deploymentInstanceDataOwnerId, actionGroup=Nothing, authorId=Nothing, deploymentId=t, templateId=(fromIntegral . fromSqlKey) deploymentInstanceDataParent}) (BearerWrapper token)) existingDeployments
handleTask _ r = do
  $(logInfo) $ (pack . show) r
  pure ()
