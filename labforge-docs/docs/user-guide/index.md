...
На топологии изображаются сети (овальные объекты) и виртуальные машины (шестиугольные объекты), а также их подключения друг к другу.
В случае нажатия на виртуальную машину, если у нее выделено имя синим цветом, вы откроете окно подключения к ней.
...
Под заголовком находятся служебные клавиши для отправки комбинаций. На изображении это кнопка отправки `Ctrl-Alt-Del` и меню вставки.

Если ваш браузер поддерживает Clipboard API (Firefox 127+, Chromium 122+), воспользуйтесь вставкой при помощи
кнопки "Вставить из буфера". Данная вставка автоматически производится из буфера обмена и поддерживает переносы строк.

В противном случае, можете ввести желаемый текст в поле рядом и воспользоваться кнопкой "Вставить". Вставка из поля ограничена тысячей символов 
и не поддерживает ряд специальных символов

!!! note "Внимание!"

    Используйте вставку с осторожностью, ее точность работы зависит от надежности сетевого соединения. При нестабильном
    соединении, вставка может "путать" символы между собой, а ее процесс может быть сложно остановить.
...
Также рядом с таблицей сетей, доступна таблица снапшотов. Здесь вы можете увидеть доступные снапшоты и создать их (если у вас достаточно прав).
Кнопка "Создать снапшот" отправляет запрос на создание снапшота. Кнопка "Обновить" обновляет список снапшотов. **Список снапшотов может обновляться
с задержкой до 10 секунд.**

Если у вас есть доступные для Вас снапшоты в списке, вы можете вызвать откат до состояния или удалить его при наличии прав. Снапшоты расположены
в хронологическом порядке (выше - более ранние), созданные Вами через платформу снапшоты имеют имя `snap1..snap100`.

![vmon3](image/vmon3.png)
