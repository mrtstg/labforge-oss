...
- agent - **полностью опциональная** область. Если на вашей ноде установлен файловый агент, ранее используемый в Labforge, вы можете его использовать для установки
статичных дисплеев
...
| Поле | Тип | Значение |
|------|-----|----------|
| name | Строка | Имя создаваемого bridge-интерфейса. Ограничено требованиями Proxmox |
| autostart | Boolean | Автозапуск интерфейса после перезагрузки сети. **Опционален, по умолчанию, true** |
| comments | Строка | Комментарий к интерфейсу. **Опционален, по умолчанию, пустая строка** |
...
Для добавления дополнительных дисков используется поле-список `disks`. Каждый диск описывается четырьмя полями:

| Поле | Тип | Значение |
|------|-----|----------|
| number | Число | Номер диска (для типа `ide` — до 4, для `sata` — до 6, для `scsi`/`virtio` — до 30) |
| type | Строка | Тип шины: `ide`, `sata`, `scsi` или `virtio` |
| size | Строка | Размер диска, например `10G` |
| storage | Строка | Имя хранилища Proxmox, куда размещается диск |
| format | Строка (опц.) | Формат диска: `raw`, `qcow2`, `vmdk` и.т.д |
...
Диски создаются как отдельные файлы с именами вида `generated-vm-<vmid>-disk-<тип>-<номер>.<формат>` и подключаются к VM через конфигурацию.
# Вызов команды
