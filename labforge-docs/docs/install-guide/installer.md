...
релизами проекта. Вы можете загрузить установщик в формате `.tar.gz` архива на [странице релизов проекта](https://github.com/mrtstg/labforge-oss/releases).
...
!!! note "Примечание"
    ...
    В зависимости от директории, у вас может не хватить прав. Используйте sudo или можете выполнить установку от лица root.
...
В случае успеха на каждый `*-sample.env` файл (кроме docker-sample.env) должен создасться файл без постфикса `-sample`. В случае
если вы планируете использовать нестандартную SDN зону (по умолчанию, `default`), потребуется в файле `jobservice.env` изменить значение поля 
`DEPLOY_SDN_ZONE`. В этой зоне Labforge будет создавать сети для подключения виртуальных машин друг к другу

Также потребуется заполнить часть значений в файле `websockify.env`:

| Значение | Комментарий | Пример |
|----------|-------------|--------|
|PROXMOX_API_URL | URL для доступа к API Proxmox, без `/api2/json` | https://192.168.1.101:8006 |
|PROXMOX_API_TOKEN | Токен для открытия websocket-сессий. Пользователь должен обладать как минимум `VM.Console` | PVEAPIToken=root@pam!token=0636f001-07d4-4423-a8c9-8c58261c747a |
|PROXMOX_CA_FILE | Путь к сертификату. По умолчанию точки проброса в контейнер нет. Значение может быть пустым. | ./rootCA.pem |
|PROXMOX_INSECURE_SKIP_VERIFY| Переключает верификацию сертификата Proxmox. Имеет значение true/false | true |
